---
tags:
  - relacion
  - fk
  - modulo/09-auditoria-reportes-y-cumplimiento
  - cross-modulo
origen: registro_acceso_datos
columna: usuario_consultor_id
destino: usuario
modulo_origen: "09"
modulo_destino: "01"
cross_modulo: true
opcional: false
cardinalidad: "no declarada en el diagrama"
---

# registro_acceso_datos.usuario_consultor_id → usuario

> **[[registro_acceso_datos]]** `.usuario_consultor_id` → **[[usuario]]**

| | |
| --- | --- |
| Entidad origen | [[registro_acceso_datos]] (módulo 09) |
| Entidad destino | [[usuario]] (módulo 01) |
| Columna | `usuario_consultor_id` — UUID |
| Cardinalidad | no declarada en el diagrama |
| Obligatoria | sí |
| Uno a uno | no |
| Cruza módulos | sí ↗ |

> [!info] Referencia entre módulos
> Esta FK conecta el módulo 09 con el 01. En los diagramas se documenta en notas al pie y, cuando es polimórfica, se valida por aplicación o trigger en lugar de con una FK física.

## Ver también

- [[09_auditoria_reportes]] — justificación de negocio del origen
- [[01_identidad_usuarios]] — justificación de negocio del destino
- [[_Relaciones]] · [[Index]]

...
-- GENERADO desde seeders/prueba/06-instrumentos-y-recargas.json — no editar a mano.
...
-- Un corresponsal habilitado con límite diario de efectivo: lo que permite ejercitar el arqueo y el tope de efectivo por punto.
INSERT INTO punto_atencion (codigo, tipo, razon_social, nit, departamento, municipio, direccion, responsable_usuario_id, limite_efectivo_diario, estado, habilitado_desde) VALUES
  ('PA-SCZ-001', 'AGENTE_CORRESPONSAL', 'Comercial El Cristo S.R.L.', '1023456789', 'SANTA CRUZ', 'Santa Cruz de la Sierra', 'Av. Cañoto esq. Ballivián 123', NULL, 20000.0, 'HABILITADO', (current_date + interval '-90 days')),
  ('PA-LPZ-001', 'AGENCIA_PROPIA', 'AportaYa S.A. — Agencia Central', '1099887766', 'LA PAZ', 'La Paz', 'Calle Potosí 1120, piso 3', NULL, 50000.0, 'HABILITADO', (current_date + interval '-120 days'))
...
INSERT INTO instrumento_fondeo (usuario_id, tipo, entidad_financiera, token_proveedor, hash_identificador, enmascarado, titular_nombre, titular_documento, titular_coincide, moneda, es_principal, estado_verificacion, metodo_verificacion, verificado_en, bloqueado_hasta) VALUES
  ...
  ((SELECT id FROM usuario WHERE codigo_publico = 'USR000004'), 'EFECTIVO', 'AGENTE CORRESPONSAL DEMO', NULL, encode(digest('demo-instrumento-04', 'sha256'), 'hex'), '****4004', 'Titular demo USR000004', '9000004', TRUE, 'BOB', TRUE, 'VERIFICADO', 'MICRODEPOSITO', now() - interval '46 days', NULL),
...
INSERT INTO orden_recarga (cuenta_billetera_id, instrumento_fondeo_id, proveedor_id, punto_atencion_id, pago_id, transaccion_id, monto_bruto, costo_proveedor, monto_acreditado, moneda, estado, referencia_externa, clave_idempotencia, solicitada_en, acreditada_en, expira_en) VALUES
  ((SELECT id FROM cuenta_billetera WHERE numero_cuenta = 'BOB-0000001'), (SELECT id FROM instrumento_fondeo WHERE hash_identificador = encode(digest('demo-instrumento-01', 'sha256'), 'hex')), (SELECT id FROM proveedor_pago WHERE codigo = 'QR_INTEROP'), NULL, NULL, NULL, 1200.0, 0.0, 1200.0, 'BOB', 'ACREDITADA', 'QR-000001-INI', 'demo-recarga-01', now() - interval '44 days', now() - interval '44 days', now() - interval '43 days'),
  ((SELECT id FROM cuenta_billetera WHERE numero_cuenta = 'BOB-0000002'), (SELECT id FROM instrumento_fondeo WHERE hash_identificador = encode(digest('demo-instrumento-02', 'sha256'), 'hex')), (SELECT id FROM proveedor_pago WHERE codigo = 'QR_INTEROP'), NULL, NULL, NULL, 1200.0, 0.0, 1200.0, 'BOB', 'ACREDITADA', 'QR-000002-INI', 'demo-recarga-02', now() - interval '44 days', now() - interval '44 days', now() - interval '43 days'),
  ((SELECT id FROM cuenta_billetera WHERE numero_cuenta = 'BOB-0000003'), (SELECT id FROM instrumento_fondeo WHERE hash_identificador = encode(digest('demo-instrumento-03', 'sha256'), 'hex')), (SELECT id FROM proveedor_pago WHERE codigo = 'QR_INTEROP'), NULL, NULL, NULL, 1200.0, 0.0, 1200.0, 'BOB', 'ACREDITADA', 'QR-000003-INI', 'demo-recarga-03', now() - interval '44 days', now() - interval '44 days', now() - interval '43 days'),
  ((SELECT id FROM cuenta_billetera WHERE numero_cuenta = 'BOB-0000004'), (SELECT id FROM instrumento_fondeo WHERE hash_identificador = encode(digest('demo-instrumento-04', 'sha256'), 'hex')), (SELECT id FROM proveedor_pago WHERE codigo = 'AGENTE_EFECTIVO'), (SELECT id FROM punto_atencion WHERE codigo = 'PA-SCZ-001'), NULL, NULL, 600.0, 0.0, 600.0, 'BOB', 'ACREDITADA', 'QR-000004-INI', 'demo-recarga-04', now() - interval '44 days', now() - interval '44 days', now() - interval '43 days'),
  ((SELECT id FROM cuenta_billetera WHERE numero_cuenta = 'BOB-0000005'), (SELECT id FROM instrumento_fondeo WHERE hash_identificador = encode(digest('demo-instrumento-05', 'sha256'), 'hex')), (SELECT id FROM proveedor_pago WHERE codigo = 'QR_INTEROP'), NULL, NULL, NULL, 2000.0, 0.0, 2000.0, 'BOB', 'ACREDITADA', 'QR-000005-INI', 'demo-recarga-05', now() - interval '44 days', now() - interval '44 days', now() - interval '43 days'),
  ((SELECT id FROM cuenta_billetera WHERE numero_cuenta = 'BOB-0000006'), (SELECT id FROM instrumento_fondeo WHERE hash_identificador = encode(digest('demo-instrumento-06', 'sha256'), 'hex')), (SELECT id FROM proveedor_pago WHERE codigo = 'QR_INTEROP'), NULL, NULL, NULL, 1200.0, 0.0, 1200.0, 'BOB', 'ACREDITADA', 'QR-000006-INI', 'demo-recarga-06', now() - interval '44 days', now() - interval '44 days', now() - interval '43 days')

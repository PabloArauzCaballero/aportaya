...
CREATE TABLE IF NOT EXISTS politica_redondeo (
...
COMMENT ON TABLE politica_redondeo IS 'Módulo 11 — Tarifas, Comisiones, Impuestos y Facturación. La política de cobro es dato, no código: se cambia con un seeder';
COMMENT ON COLUMN politica_redondeo.id IS 'PK';
COMMENT ON COLUMN politica_redondeo.codigo IS 'UQ';
COMMENT ON COLUMN politica_redondeo.modo IS 'CK';
COMMENT ON COLUMN politica_redondeo.aplica_a IS 'CK';

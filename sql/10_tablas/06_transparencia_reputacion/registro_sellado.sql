...
COMMENT ON TABLE registro_sellado IS 'Módulo 06 — Transparencia y Reputación. [append-only] Que nadie tenga que "creerle" al organizador';
COMMENT ON COLUMN registro_sellado.id IS 'PK';
COMMENT ON COLUMN registro_sellado.bloque_id IS 'FK, IDX';
COMMENT ON COLUMN registro_sellado.tipo_entidad IS 'CK';
COMMENT ON COLUMN registro_sellado.entidad_id IS 'IDX, polimorfica';

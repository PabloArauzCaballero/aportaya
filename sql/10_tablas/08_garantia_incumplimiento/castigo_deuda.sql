...
CREATE TABLE IF NOT EXISTS castigo_deuda (
...
COMMENT ON TABLE castigo_deuda IS 'Módulo 08 — Garantía, Incumplimiento, Cobranza y Sanciones. El grupo no se detiene, pero la deuda no se perdona sola';
COMMENT ON COLUMN castigo_deuda.id IS 'PK';
COMMENT ON COLUMN castigo_deuda.deuda_id IS 'FK, UQ';
COMMENT ON COLUMN castigo_deuda.aprobado_por IS 'FK';
COMMENT ON COLUMN castigo_deuda.asiento_contable_id IS 'FK, NULL, M3';
COMMENT ON COLUMN castigo_deuda.motivo IS 'CK';

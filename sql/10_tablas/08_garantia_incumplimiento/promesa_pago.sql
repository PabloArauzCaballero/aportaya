...
COMMENT ON TABLE promesa_pago IS 'Módulo 08 — Garantía, Incumplimiento, Cobranza y Sanciones. El grupo no se detiene, pero la deuda no se perdona sola';
COMMENT ON COLUMN promesa_pago.id IS 'PK';
COMMENT ON COLUMN promesa_pago.gestion_id IS 'FK, IDX';
COMMENT ON COLUMN promesa_pago.fecha_prometida IS 'IDX';
COMMENT ON COLUMN promesa_pago.estado IS 'CK, IDX';
COMMENT ON COLUMN promesa_pago.registrada_por IS 'FK, NULL';
COMMENT ON COLUMN promesa_pago.evaluada_en IS 'NULL';

...
COMMENT ON TABLE prueba_control IS 'Módulo 12 — Cumplimiento Regulatorio y Consumidor Financiero. Que una inspección se responda con consultas, no armando carpetas';
COMMENT ON COLUMN prueba_control.id IS 'PK';
COMMENT ON COLUMN prueba_control.control_id IS 'FK, IDX';
COMMENT ON COLUMN prueba_control.ejecutada_por IS 'FK';
COMMENT ON COLUMN prueba_control.periodo IS 'UQ+control_id';
COMMENT ON COLUMN prueba_control.resultado IS 'CK, IDX';
COMMENT ON COLUMN prueba_control.evidencia_url IS 'NULL';

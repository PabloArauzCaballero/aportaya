...
CREATE TABLE IF NOT EXISTS umbral_reporte_uif (
...
COMMENT ON TABLE umbral_reporte_uif IS 'Módulo 12 — Cumplimiento Regulatorio y Consumidor Financiero. Que una inspección se responda con consultas, no armando carpetas';
COMMENT ON COLUMN umbral_reporte_uif.id IS 'PK';
COMMENT ON COLUMN umbral_reporte_uif.formulario IS 'CK, UQ+concepto_operacion+es_acumulado+vigente_desde';
COMMENT ON COLUMN umbral_reporte_uif.concepto_operacion IS 'CK';
COMMENT ON COLUMN umbral_reporte_uif.ventana_dias_calendario IS 'NULL';
COMMENT ON COLUMN umbral_reporte_uif.vigente_hasta IS 'NULL';
COMMENT ON COLUMN umbral_reporte_uif.activo IS 'IDX';

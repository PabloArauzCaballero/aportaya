...
CREATE TABLE IF NOT EXISTS movimiento_billetera (
...
COMMENT ON TABLE movimiento_billetera IS 'Módulo 10 — Billetera, Custodia y Dinero Electrónico. [append-only] El saldo no se guarda: se deriva, y todos los días cuadra contra el banco';
COMMENT ON COLUMN movimiento_billetera.id IS 'PK';
COMMENT ON COLUMN movimiento_billetera.transaccion_id IS 'FK, IDX';
COMMENT ON COLUMN movimiento_billetera.cuenta_billetera_id IS 'FK, IDX';
COMMENT ON COLUMN movimiento_billetera.sentido IS 'CK: DEBITO|CREDITO';
COMMENT ON COLUMN movimiento_billetera.monto IS 'CK: > 0';
COMMENT ON COLUMN movimiento_billetera.registrado_en IS 'IDX';

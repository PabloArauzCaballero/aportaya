...
CREATE TABLE IF NOT EXISTS intento_validacion_token (
...
COMMENT ON TABLE intento_validacion_token IS 'Módulo 01 — Identidad, Usuarios y Seguridad. Saber con certeza a quién le estás confiando plata ajena';
COMMENT ON COLUMN intento_validacion_token.id IS 'PK';
COMMENT ON COLUMN intento_validacion_token.token_id IS 'FK, IDX';
COMMENT ON COLUMN intento_validacion_token.resultado IS 'CK';
COMMENT ON COLUMN intento_validacion_token.huella_dispositivo IS 'NULL';

...
CREATE TABLE IF NOT EXISTS disputa_pago (
...
COMMENT ON TABLE disputa_pago IS 'Módulo 03 — Aportes, Pagos QR y Conciliación. Que "pagué" signifique "el banco lo confirmó"';
COMMENT ON COLUMN disputa_pago.id IS 'PK';
COMMENT ON COLUMN disputa_pago.pago_id IS 'FK, IDX';
COMMENT ON COLUMN disputa_pago.tipo IS 'CK';
COMMENT ON COLUMN disputa_pago.estado IS 'CK';
COMMENT ON COLUMN disputa_pago.resuelta_en IS 'NULL';
